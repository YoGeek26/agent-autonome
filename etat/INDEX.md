...
| Coût réel d'un réveil | Moyenne **3,01 USD** sur 5 réveils, 134,96 restants au 2026-08-11 15:11. **Le chiffre à retenir est un nombre de réveils, pas de jours** : ≈ **44 tentatives**, ou ≈ 67 si le coût retombe vers 2,00. L'opérateur avertit que cette moyenne est **faussée à la hausse** : presque tous ces réveils sont des réveils qu'il a déclenchés pendant l'installation, en Opus sans triage, « pas ton régime de croisière » — il corrige son superviseur. Donc traiter 3,01 comme un plafond, pas comme une prévision. `jours_restants` et `usd_par_jour` dans `cout.json` sont des artefacts tant que l'ancienneté se compte en heures (0 et 172). | `cout.json` du 2026-08-11 15:11 ; message opérateur 2026-08-11 15:14 |
| **Aucune cadence ne couvre les 90 jours** | Fait arithmétique **arrêté — ne pas le refaire à chaque réveil** : plafond de cadence **1440 min** (relevé par l'opérateur le 2026-08-11 15:00, il était de 360). Une fois par jour = 90 réveils ≈ 270 USD contre 134,96 restants → à sec vers le **25 septembre 2026**, pour une échéance au 9 novembre. Il faudrait un réveil toutes les 43 h, au-dessus du plafond. Donc **le plafond n'est plus la contrainte, le budget l'est**, et l'arbitrage n'est jamais « tenir jusqu'au bout » mais « combien de tentatives *distinctes* ». Corollaire opérationnel : ne jamais serrer la cadence pour surveiller quelque chose que je ne peux pas réparer. | réveils #5 et #6, `cout.json` |
...
| **Première requête réellement extérieure** | 2026-08-11 15:12:00 UTC, IP `100.53.201.212` (sans rapport avec le VPS), user-agent navigateur iPhone, séquence complète d'un chargement : `/` 200 → `style.css` 200 → favicon ; deux chargements de `/` à 14 s d'écart. **Identité inconnue** : vraisemblablement l'opérateur (l'heure colle à son message de 15:14), non confirmé, question posée. C'est la seule preuve de routage extérieur que possède l'expérience. | registre 2026-08-11 15:12:00, `bin/frequentation` (réveil #6) |
